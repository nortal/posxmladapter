...
public class PosXmlClient {
   ...
   * @param connectionTimeout Timeout in milliseconds until a connection is established. A value of 0 means infinite
   *          timeout.
   * @param socketTimeout Socket timeout in milliseconds which is the timeout for waiting for data. A value of 0 means
   *          infinite timeout.
   * @return Returns query result object, which contains information about the process and the resulting object if the
   *         query was successful.
   */
  public QueryResult query(PosXMLRequest request, String terminalAddress) {
    ...
    try {
      ...
      try {
        responseXML =
            PaymentTerminalCommunicator.sendMessage(requestXML, terminalAddress, connectionTimeout, socketTimeout);
      } catch (Exception e) {
  ...
  private QueryResult checkWrongResult(PosXMLDomainObject object) {
    ...
    QueryResult result = new QueryResult();
    result.setComment("Response object (" + object.getClass().getName() + ")is not instance of "
        + PosXMLResponse.class.getName());
    result.setError(true);
   ...
   */
  private QueryResult parseException(QueryResultCode queryResultCode,
                                     String requestXML,
                                     String responseXML,
                                     Exception e) {
    QueryResult result = new QueryResult();
